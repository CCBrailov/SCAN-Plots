...
testdataset = dataset(glob.glob(f'data/MATRiX/full.txt'), args)
...
for b, batch in enumerate(testloader):
	if b >= 224: 
		imageio.mimwrite(f'plots/MATRiX/movie.gif', img_array, fps=2)
		break
	...
	plt.xticks([i * 2 for i in range(50)])
	plt.yticks([i * 2 for i in range(50)])
	plt.xlabel(' ')
	plt.ylabel(' ')
	plt.tight_layout()
	plt.xlim([0, 25])
